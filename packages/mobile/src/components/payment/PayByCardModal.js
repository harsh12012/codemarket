...
const { StripeBridge, StripePayments } = NativeModules;
  ...
  const onPress = async () => {
    try {
      if (payload.year && payload.ccnumber && payload.month && payload.cvc) {
        ...
        if (Platform.OS === 'ios') {
          StripeBridge.confirmPaymentIntent(
            secret,
            ccnumber,
            month,
            year,
          ...
          await StripePayments.confirmPaymentIntent(secret, cardDetails);
          await onComplete(
            data.stripeCreatePaymentIntent.id,
            data.stripeCreatePaymentIntent.transferGroup
          );
